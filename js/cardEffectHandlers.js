...
function blocked(state, r, c) {
  ...
  const k = sk(r, c);
  if (state.meta.collapsed?.has(k)) return true;
...
const EFFECTS = {
  ...
  destroy_unshielded(state, color, picks) { const [r,c]=p0(picks); if(!kill(state,r,c,color)) return fail('Invalid or shielded'); return ok(); },
  ...
  revive(state, color, picks) { const [r,c]=p0(picks); const cap=state.captured[color]; if(!cap.length) return fail('No captures'); if(!backRow(color).includes(r)||!emptyDark(state,r,c)) return fail(); const data=cap.pop(); const p=createPiece(color,r,c,data.king); state.board[r][c]=p; return ok(); },
  ...
  freeze_2(state, color, picks) { const [r,c]=p0(picks); const p=at(state,r,c); if(!p||p.color===color) return fail(); p.frozenTurns=2; return ok(); },
  root_2(state, color, picks) { const [r,c]=p0(picks); const p=at(state,r,c); if(!p||p.color===color) return fail(); p.rooted=2; return ok(); },
  ...
  collapse(state, color, picks) { const [r,c]=p0(picks); if(!isDarkSquare(r,c)) return fail(); state.meta.collapsed.add(sk(r,c)); const p=at(state,r,c); if(p){ removePiece(state.board,r,c); for(let r2=0;r2<SIZE;r2++) for(let c2=0;c2<SIZE;c2++) if(emptyDark(state,r2,c2)){ state.board[r2][c2]=p; p.row=r2; p.col=c2; break;}} return ok(); },
  ...
  exile_king(state, color, picks) { const [r,c]=p0(picks); const p=at(state,r,c); if(!p||p.color===color||!p.king) return fail(); const rows=backRow(p.color); const spots=[]; for(const r of rows) for(let c=0;c<SIZE;c++) if(emptyDark(state,r,c)) spots.push([r,c]); if(!spots.length) return fail(); const [tr,tc]=spots[Math.floor(Math.random()*spots.length)]; movePiece(state.board,p.row,p.col,tr,tc); return ok(); },
  ...
  wild_magic(state, color, picks) { const card=drawRandomCard(); return applyEffect(state,color,card.effect,[]); },
