import{O as e,q as t}from"./storage-BO336-Gl.js";import{s as n}from"./mageTitles-BPvsmJqw.js";import{a as r}from"./auth-CWl6fVMK.js";import{a as i,l as a,u as o}from"./index-D3QLIpf1.js";import{t as s}from"./profileStatIcons-DJomtKMV.js";function c(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function l(e,t,{clickable:n=!1,userId:r=``}={}){let i=e?.equipped||{},s=(t||`P`).charAt(0).toUpperCase(),l=o(i.avatar)||`<span class="profile-avatar-fallback">${c(s)}</span>`,u=`<span class="profile-avatar-stack ${a(i.frame)}"><span class="profile-avatar-inner">${l}</span></span>`;return n&&r?`<button type="button" class="pvp-room-host-profile" data-view-profile="${c(r)}" data-profile-name="${c(t)}" aria-label="View ${c(t)}'s profile">${u}</button>`:`<span class="pvp-room-host-profile pvp-room-host-profile--static" aria-hidden="true">${u}</span>`}function u(e){return`
    <div class="profile-hero-stats public-profile-modal__stats" aria-label="Player statistics">
      ${[{key:`pvp`,label:`PvP wins`,value:e.pvpWins},{key:`adventure`,label:`Floors cleared`,value:e.adventureFloorsCleared},{key:`spells`,label:`Spells played`,value:e.spellsPlayed}].map(e=>`
        <article class="profile-stat-card profile-stat-card--${e.key}">
          <span class="profile-stat-card__label">${c(e.label)}</span>
          <span class="profile-stat-card__icon">${s[e.key]}</span>
          <span class="profile-stat-card__value">${e.value}</span>
        </article>`).join(``)}
    </div>`}function d({username:e,cosmetics:t,stats:r}){let s=t.equipped||{},l=(e||`P`).charAt(0).toUpperCase(),d=o(s.avatar)||`<span class="profile-avatar-fallback">${c(l)}</span>`,f=n({cosmetics:t},{compact:!1});return`
