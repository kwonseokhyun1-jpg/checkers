import{$ as e,A as t,C as n,D as r,E as i,G as a,L as o,M as s,N as c,O as l,P as u,Q as d,S as f,U as p,W as m,_ as h,b as g,k as _,m as v,nt as y,ot as ee,v as b,w as x,x as S,y as C}from"./storage-np6UGASY.js";import{a as w,c as T,l as te,o as E,t as D,u as O}from"./mageTitles-DcAwBwLH.js";import{a as k,c as A,o as ne}from"./auth-DFecQAOy.js";import{U as re,c as j,i as M,l as N,n as P,o as F,r as I,s as L,t as R,u as z}from"./index-CV2-qlXG.js";import{playCosmeticOpenAnimation as B}from"./cosmeticOpenAnimation-bNH1l5a_.js";import{notifyUnlockTutorial as V}from"./tutorialUnlocks-CIaHHVlb.js";var H=null;function U(){H!=null&&(clearInterval(H),H=null)}function W(e){if(!e)return;U();let t=()=>{e.textContent=`Resets in: ${S(x())}`};t(),H=setInterval(t,1e3)}function G(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var K={common:`rarity-common`,uncommon:`rarity-uncommon`,rare:`rarity-rare`,epic:`rarity-epic`,legendary:`rarity-legendary`};function q(e){if(e===`pieceSkin`)return`Piece skins`;if(e===`frame`)return`Frames`;let t=L(e);return t.endsWith(`s`)?t:`${t}s`}function J(e){let t=T(e);return t?O(t):``}var Y={pvp:`<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <div class="profile-hero-card">
      ...
      <div class="profile-achievement-card__head">
        <h4 class="profile-achievement-card__title">${G(r.title)}</h4>
        ${i?`<span class="profile-achievement-card__reward mage-title-tag mage-title-tag--glow-${i.glow} ${w[i.rarity]||``}">[${G(i.display)}]</span>`:``}
      </div>
      <p class="profile-achievement-card__desc">${G(r.description)}</p>
      ...
      <span class="profile-achievement-card__status">${y}</span>`,f){let i=document.createElement(`button`);i.type=`button`,i.className=`btn-primary profile-achievement-card__claim`,i.textContent=`Claim Title`,i.addEventListener(`click`,()=>{t(e,r.id).success&&(v(e),Z(e,n,{onTitleChanged:a}),a?.())}),m.appendChild(i)}n.appendChild(m)}}function Q(e,t,{onCurrencyChange:r}={}){if(!t)return;t.innerHTML=``;let i=[...f(e)].sort((t,n)=>{let r=e=>e.canClaim?0:e.claimed?2:1,i=r(t)-r(n);return i===0?C(e,n.templateId)-C(e,t.templateId):i});for(let{template:a,templateId:o,complete:s,claimed:c,canClaim:l}of i){if(!a)continue;let i=!s&&!c,u=document.createElement(`article`);u.className=[`profile-achievement-card`,`daily-quest-card`,i?`profile-achievement-card--locked`:``,s?`profile-achievement-card--complete`:``,c?`profile-achievement-card--claimed`:``,l?`profile-achievement-card--claimable`:``].filter(Boolean).join(` `);let d=b(e,o),f=a.target?Math.min(100,Math.round(n(e,o)/a.target*100)):0,p=g(o),m=a.reward.currency===`stars`?`daily-quest-card__reward--stars`:`daily-quest-card__reward--gems`,_=c?`Claimed`:l?`Claim reward`:s?`Complete`:`In progress`;if(u.innerHTML=`
      <div class="profile-achievement-card__head">
    ...
    <section class="panel game-panel profile-panel">
      ...
      <div id="profile-section-cosmetics" class="profile-section-panel">
        <div class="profile-cosmetic-filters" role="tablist" aria-label="Cosmetic category">
          ${a.map(e=>`<button type="button" class="profile-filter-btn" role="tab" data-cos-filter="${e}">${q(e)}</button>`).join(``)}
        </div>
  ...
  `;let l=n.querySelector(`#profile-cosmetic-grid`),u=`avatar`,f=``,p=()=>{let r=e(t);n.querySelector(`#profile-banner-preview`).style.background=M(r.equipped.banner);let a=n.querySelector(`#profile-avatar-preview`),o=(f||`P`).charAt(0).toUpperCase();a&&(a.innerHTML=N(r.equipped.avatar)||`<span class="profile-avatar-fallback">${G(o)}</span>`);let s=n.querySelector(`#profile-avatar-stack`);s&&(s.className=`profile-avatar-stack ${j(r.equipped.frame)}`);let c=n.querySelector(`#profile-hero-username`);c&&(c.textContent=f||`Player`);let l=n.querySelector(`.profile-hero-card__title-badge`),u=J(t);if(u)if(l)l.innerHTML=u;else{let e=n.querySelector(`.profile-hero-card__name-row`);if(e){let t=document.createElement(`span`);t.className=`profile-hero-card__title-badge`,t.innerHTML=u,e.appendChild(t)}}else l&&l.remove();i?.()},h=()=>{let e=t.cosmetics?.owned?.[u]||[];l.innerHTML=``;for(let n of e){let e=m[n];if(!e)continue;let r=t.cosmetics.equipped[u]===n,i=document.createElement(`button`);i.type=`button`,i.className=`profile-cosmetic-card ${K[e.rarity]||``} ${r?`profile-cosmetic-card--equipped`:``}`;let a=z(n,e.type);i.innerHTML=`
        ...
        <span class="profile-cosmetic-card__action">${r?`Equipped`:`Equip`}</span>`,i.addEventListener(`click`,()=>{r||d(t,u,n).success&&(v(t),h(),p(),V(`cosmetic-equipped`,{type:u,id:n}))}),l.appendChild(i)}e.length||(l.innerHTML=`<p class="muted">Open cosmetic boxes in the Shop to unlock items.</p>`)};for(let e of n.querySelectorAll(`.profile-filter-btn`))e.addEventListener(`click`,()=>{u=e.dataset.cosFilter,n.querySelectorAll(`.profile-filter-btn`).forEach(t=>t.classList.toggle(`active`,t===e)),h()});n.querySelector(`.profile-filter-btn`)?.classList.add(`active`),h();let g=()=>{let e=n.querySelector(`#profile-title-grid`);if(!e)return;e.innerHTML=``;let r=T(t);for(let n of D){let a=te(t,n.id),o=r===n.id,s=document.createElement(`button`);s.type=`button`,s.disabled=!a,s.className=[`profile-title-card`,w[n.rarity]||``,`profile-title-card--glow-${n.glow}`,a?``:`profile-title-card--locked`,o?`profile-title-card--equipped`:``].filter(Boolean).join(` `),s.innerHTML=`
        <span class="profile-title-card__tag mage-title-tag mage-title-tag--glow-${n.glow} ${w[n.rarity]||``}">[${G(n.display)}]</span>
        <span class="profile-title-card__rarity">${n.rarity}</span>
        <span class="profile-title-card__action">${o?`Equipped`:a?`Equip`:`Locked`}</span>`,s.addEventListener(`click`,()=>{a&&E(t,o?null:n.id).success&&(v(t),g(),p(),i?.())}),e.appendChild(s)}},_=e=>{n.querySelectorAll(`.profile-section-tab`).forEach(t=>{t.classList.toggle(`active`,t.dataset.profileSection===e)});let t=n.querySelector(`#profile-section-cosmetics`);t&&(t.classList.toggle(`hidden`,e!==`cosmetics`),t.hidden=e!==`cosmetics`);let r=n.querySelector(`#profile-section-titles`);r&&(r.classList.toggle(`hidden`,e!==`titles`),r.hidden=e!==`titles`),e===`titles`&&g()};for(let e of n.querySelectorAll(`.profile-section-tab`))e.addEventListener(`click`,()=>_(e.dataset.profileSection));g(),c&&(async()=>{try{f=(await k(s.id))?.username||s.user_metadata?.display_name||``,p()}catch(e){console.warn(`Could not load profile username`,e)}})()}export{R as headerProfileAvatarHtml,oe as renderAchievementsTab,X as renderCosmeticBoxes,se as renderProfileTab,$ as renderQuestsTab,P as resolveDisplayUsername};