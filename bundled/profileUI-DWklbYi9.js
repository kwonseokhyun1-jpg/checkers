import{$ as e,C as t,F as n,I as r,O as i,P as a,Q as o,T as s,U as c,W as l,Z as u,_ as d,b as f,et as p,it as m,m as h,nt as g,ot as _,q as v,rt as y,tt as b,v as x,w as S,x as C,y as w}from"./storage-CKPuNPOL.js";import{a as T,c as E,d as D,l as O,o as k,t as A,u as j}from"./mageTitles-BPvsmJqw.js";import{a as ee,c as te,o as ne}from"./auth-BZfV7vJW.js";import{W as re,c as M,i as N,l as P,n as F,o as I,r as L,s as R,t as z,u as B}from"./index-D_fA3Vpc.js";import{playCosmeticOpenAnimation as V}from"./cosmeticOpenAnimation-B0EnHBmV.js";import{notifyUnlockTutorial as H}from"./tutorialUnlocks-SDTMxwFD.js";var U=null;function W(){U!=null&&(clearInterval(U),U=null)}function G(e){if(!e)return;W();let t=()=>{e.textContent=`Resets in: ${b(m())}`};t(),U=setInterval(t,1e3)}function K(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var q={common:`rarity-common`,uncommon:`rarity-uncommon`,rare:`rarity-rare`,epic:`rarity-epic`,legendary:`rarity-legendary`};function J(e){if(e===`pieceSkin`)return`Piece skins`;if(e===`frame`)return`Frames`;let t=R(e);return t.endsWith(`s`)?t:`${t}s`}function Y(e){let t=E(e);return t?D(t):``}var ie={pvp:`<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  </svg>`};function ae(e){let t=i(e);return`
    ...
    <div class="profile-hero-card">
      ...
      ${ae(t)}
    </div>`}function X(e,t,{logEl:n,onGemsChange:r,onOpened:i,cosmeticsUnlocked:o=!0}={}){if(t){if(t.innerHTML=``,!o){t.innerHTML=`<p class="vault-locked-msg muted">${_}</p>`;return}for(let s of a){let a=L[s.id]||L.style_crate,c=e.gems>=s.cost,l=document.createElement(`article`);l.className=`chest-card chest-card--${a.visual} cosmetic-box-card cosmetic-box-card--${s.id}${c?``:` chest-card--locked`}`,l.innerHTML=`
      <div class="chest-card__aura" aria-hidden="true"></div>
      <div class="chest-card__visual">${I(s.id)}</div>
      <div class="chest-card__body">
        <span class="chest-card__tier">${a.label}</span>
        <h3 class="chest-card__name">${s.name}</h3>
        <p class="chest-card__tagline">${re(s.weights)}</p>
        <ul class="chest-card__stats">
          <li><strong>${s.pulls}</strong> cosmetics</li>
      ...
      <button type="button" class="btn-primary btn-open-cosmetic chest-card__btn" data-box="${s.id}">
        ${c?`Open`:`Need more gems`}
      </button>`;let u=l.querySelector(`.btn-open-cosmetic`);u.disabled=!c,u.addEventListener(`click`,async()=>{if(u.disabled)return;let c=v(e,s.id);if(!c.success){n&&(n.textContent=c.message);return}h(e),r?.(),u.disabled=!0,await V({boxId:s.id,boxLabel:a.label,pulls:c.pulls});let l=c.pulls.map(e=>`${e.name}${e.duplicate?` (duplicate)`:``}`).join(`, `);n&&(n.textContent=`Opened ${s.name}: ${l}${c.bonusGems?` · +${c.bonusGems} gems from duplicates`:``}`),i?.(c),H(`cosmetic-box-opened`,{boxId:s.id,pulls:c.pulls}),X(e,t,{logEl:n,onGemsChange:r,onOpened:i,cosmeticsUnlocked:o})}),t.appendChild(l)}}}function Z(e,n,{onTitleChanged:r}={}){if(!n)return;n.innerHTML=``;let i=[...d].sort((n,r)=>{let i=n=>f(e,n)?0:t(e,n)?2:1,a=i(n.id)-i(r.id);if(a!==0)return a;let o=x(e,r.id)-x(e,n.id);return o===0?n.title.localeCompare(r.title):o});for(let a of i){let i=w(a.id),o=S(e,a.id),c=t(e,a.id),l=f(e,a.id),u=!o&&!c,d=document.createElement(`article`);d.className=[`profile-achievement-card`,u?`profile-achievement-card--locked`:``,o?`profile-achievement-card--complete`:``,c?`profile-achievement-card--claimed`:``,l?`profile-achievement-card--claimable`:``].filter(Boolean).join(` `);let p=s(e,a.id),m=a.target?Math.min(100,Math.round((e.achievements?.progress?.[a.id]||0)/a.target*100)):0,g=c?`Unlocked`:l?`Claim Title`:o?`Complete`:`In progress`;if(d.innerHTML=`
      ...
      <p class="profile-achievement-card__progress-text">${K(p)}</p>
      <span class="profile-achievement-card__status">${g}</span>`,l){let t=document.createElement(`button`);t.type=`button`,t.className=`btn-primary profile-achievement-card__claim`,t.textContent=`Claim Title`,t.addEventListener(`click`,()=>{C(e,a.id).success&&(h(e),Z(e,n,{onTitleChanged:r}),r?.())}),d.appendChild(t)}n.appendChild(d)}}function Q(t,n,{onCurrencyChange:r}={}){if(!n)return;n.innerHTML=``;let i=[...g(t)].sort((n,r)=>{let i=e=>e.canClaim?0:e.claimed?2:1,a=i(n)-i(r);return a===0?e(t,r.templateId)-e(t,n.templateId):a});for(let{template:e,templateId:a,complete:s,claimed:c,canClaim:l}of i){if(!e)continue;let i=!s&&!c,d=document.createElement(`article`);d.className=[`profile-achievement-card`,`daily-quest-card`,i?`profile-achievement-card--locked`:``,s?`profile-achievement-card--complete`:``,c?`profile-achievement-card--claimed`:``,l?`profile-achievement-card--claimable`:``].filter(Boolean).join(` `);let f=o(t,a),m=e.target?Math.min(100,Math.round(y(t,a)/e.target*100)):0,g=p(a),_=e.reward.currency===`stars`?`daily-quest-card__reward--stars`:`daily-quest-card__reward--gems`,v=c?`Claimed`:l?`Claim reward`:s?`Complete`:`In progress`;if(d.innerHTML=`
      <div class="profile-achievement-card__head">
        <h4 class="profile-achievement-card__title">${K(e.title)}</h4>
        <span class="daily-quest-card__reward ${_}">${K(g)}</span>
      </div>
      <p class="profile-achievement-card__desc">${K(e.description)}</p>
      <div class="profile-achievement-card__progress" role="progressbar" aria-valuenow="${m}" aria-valuemin="0" aria-valuemax="100">
        <div class="profile-achievement-card__progress-fill" style="width:${m}%"></div>
      </div>
      <p class="profile-achievement-card__progress-text">${K(f)}</p>
      <span class="profile-achievement-card__status">${v}</span>`,l){let i=document.createElement(`button`);i.type=`button`,i.className=`btn-primary profile-achievement-card__claim`,i.textContent=e.reward.currency===`stars`?`Claim stars`:`Claim gems`,i.addEventListener(`click`,()=>{u(t,a).success&&(h(t),Q(t,n,{onCurrencyChange:r}),r?.())}),d.appendChild(i)}n.appendChild(d)}}function $(e,t,{onTitleChanged:n,onCurrencyChange:r}={}){if(!t)return;W(),t.innerHTML=`
    ...
    <section class="panel game-panel profile-panel">
      ...
      <div id="profile-section-cosmetics" class="profile-section-panel">
        <div class="profile-cosmetic-filters" role="tablist" aria-label="Cosmetic category">
          ${r.map(e=>`<button type="button" class="profile-filter-btn" role="tab" data-cos-filter="${e}">${J(e)}</button>`).join(``)}
        </div>
  ...
  `;let d=t.querySelector(`#profile-cosmetic-grid`),f=`avatar`,p=``,m=()=>{let n=l(e);t.querySelector(`#profile-banner-preview`).style.background=N(n.equipped.banner);let r=t.querySelector(`#profile-avatar-preview`),i=(p||`P`).charAt(0).toUpperCase();r&&(r.innerHTML=P(n.equipped.avatar)||`<span class="profile-avatar-fallback">${K(i)}</span>`);let o=t.querySelector(`#profile-avatar-stack`);o&&(o.className=`profile-avatar-stack ${M(n.equipped.frame)}`);let s=t.querySelector(`#profile-hero-username`);s&&(s.textContent=p||`Player`);let c=t.querySelector(`.profile-hero-card__title-badge`),u=Y(e);if(u)if(c)c.innerHTML=u;else{let e=t.querySelector(`.profile-hero-card__name-row`);if(e){let t=document.createElement(`span`);t.className=`profile-hero-card__title-badge`,t.innerHTML=u,e.appendChild(t)}}else c&&c.remove();a?.()},g=()=>{let t=e.cosmetics?.owned?.[f]||[];d.innerHTML=``;for(let r of t){let t=n[r];if(!t)continue;let i=e.cosmetics.equipped[f]===r,a=document.createElement(`button`);a.type=`button`,a.className=`profile-cosmetic-card ${q[t.rarity]||``} ${i?`profile-cosmetic-card--equipped`:``}`;let o=B(r,t.type);a.innerHTML=`
