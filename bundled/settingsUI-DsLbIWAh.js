import{_ as e,a as t,b as n,c as r,g as i,i as a,m as o,n as s,o as c,r as l,u,y as d}from"./auth-DHasSuFN.js";import{V as f,_ as p,g as m,m as h,q as g}from"./index-BX4eR634.js";var _=`https://sites.google.com/view/arcane-checkers/home`;function v(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function y(){let e=m();return`
    ...
    <div class="profile-account">
      ...
      </div>`}function S(e){let t=e.querySelector(`#settings-music`),n=e.querySelector(`#settings-music-volume`),r=e.querySelector(`#settings-sfx`),i=e.querySelector(`#settings-sfx-volume`),a=e.querySelector(`#settings-haptics`),o=e=>{p(e),h()};t?.addEventListener(`change`,()=>o({musicEnabled:t.checked})),n?.addEventListener(`input`,()=>o({musicVolume:Number(n.value)/100})),r?.addEventListener(`change`,()=>o({sfxEnabled:r.checked})),i?.addEventListener(`input`,()=>o({sfxVolume:Number(i.value)/100})),a?.addEventListener(`change`,()=>o({hapticsEnabled:a.checked}))}function C(f,{onUsernameChanged:p}={}){if(!f)return;let m=c(),h=r()&&!!m;f.innerHTML=`
    <section class="panel game-panel settings-page-panel">
      ...
      <section class="settings-group settings-account-section" aria-labelledby="settings-account-heading">
        ...
        ${x({signedIn:h,username:``,email:m?.email||``})}
      </section>
    </section>
  `,S(f),f.querySelector(`#settings-sign-out`)?.addEventListener(`click`,()=>{o()});let _=f.querySelector(`#settings-delete-account`),v=f.querySelector(`#settings-delete-account-status`);if(_?.addEventListener(`click`,async()=>{if(await g(`This permanently deletes your account, cloud save, decks, and username. You will need a new account to play again.`,{title:`Delete account?`,confirmLabel:`Delete account`,cancelLabel:`Keep account`,destructive:!0})){v.textContent=``,v.classList.remove(`profile-delete-account__status--error`),_.disabled=!0;try{await a()}catch(e){v.textContent=e.message||`Could not delete account`,v.classList.add(`profile-delete-account__status--error`),_.disabled=!1}}}),!h)return;let C=f.querySelector(`#settings-username-display`),w=f.querySelector(`#settings-username-editor`),T=f.querySelector(`#settings-username-change`),E=f.querySelector(`#settings-username-cancel`),D=f.querySelector(`#settings-username`),O=f.querySelector(`#settings-username-hint`),k=f.querySelector(`#settings-username-status`),A=f.querySelector(`#settings-username-save`),j=null,M=``,N=null,P=e=>{C&&(C.textContent=e||`—`)},F=(e,t=``)=>{O&&(O.textContent=e||``,O.classList.remove(`auth-username-hint--ok`,`auth-username-hint--bad`),t===`ok`&&O.classList.add(`auth-username-hint--ok`),t===`bad`&&O.classList.add(`auth-username-hint--bad`))},I=(e,t=!1)=>{k&&(k.textContent=e||``,k.classList.toggle(`profile-username-status--error`,t))},L=()=>{clearTimeout(j),j=setTimeout(async()=>{let e=D?.value?.trim()||``;if(!e){F(``);return}let t=n(e);if(t){F(t,`bad`);return}if(e.toLowerCase()===M.toLowerCase()){F(`Current username`,`ok`);return}F(`Checking…`);try{if(!await u(e,m.id)){let t=await i(e);F(t?`Taken — try "${t}"`:`That username is taken`,`bad`),t&&O&&(O.dataset.suggestion=t);return}O&&delete O.dataset.suggestion,F(`Available`,`ok`)}catch{F(``)}},350)},R=e=>{w?.classList.toggle(`hidden`,!e),w&&(w.hidden=!e),T?.classList.toggle(`hidden`,e),e?(D&&(D.value=M,D.focus(),D.select()),I(``),L()):D&&(D.value=M,F(``),I(``))};O?.addEventListener(`click`,()=>{let e=O?.dataset?.suggestion;!e||!D||(D.value=e,delete O.dataset.suggestion,L())}),D?.addEventListener(`input`,L),T?.addEventListener(`click`,()=>R(!0)),E?.addEventListener(`click`,()=>R(!1)),(async()=>{try{N=await t(m.id),M=N?.username||m.user_metadata?.display_name||``,P(M),p?.(M),D&&M&&(D.value=M);let e=s(N);e.ok||F(e.message,`bad`)}catch(e){console.warn(`Could not load profile username`,e)}})(),A?.addEventListener(`click`,async()=>{let n=D?.value?.trim()||``;I(``),A.disabled=!0;try{let r=s(N);if(!r.ok)throw Error(r.message);let i=await e(n);N=await t(m.id),M=i,P(M),R(!1),p?.(i)}catch(e){I(e.message||`Could not save username`,!0)}finally{A.disabled=!1}});let z=f.querySelector(`#settings-password-editor`),B=f.querySelector(`#settings-password-change`),V=f.querySelector(`#settings-password-cancel`),H=f.querySelector(`#settings-password-current`),U=f.querySelector(`#settings-password-new`),W=f.querySelector(`#settings-password-confirm`),G=f.querySelector(`#settings-password-save`),K=f.querySelector(`#settings-password-status`),q=f.querySelector(`#settings-password-feedback`),J=(e,t=!1)=>{K&&(K.textContent=e||``,K.classList.toggle(`profile-username-status--error`,t))},Y=(e,t=!1)=>{q&&(q.textContent=e||``,q.classList.toggle(`profile-username-status--error`,t))},X=()=>{H&&(H.value=``),U&&(U.value=``),W&&(W.value=``)},Z=e=>{z?.classList.toggle(`hidden`,!e),z&&(z.hidden=!e),B?.classList.toggle(`hidden`,e),e?(X(),J(``),H?.focus()):(X(),J(``))};B?.addEventListener(`click`,()=>{Y(``),Z(!0)}),V?.addEventListener(`click`,()=>Z(!1)),G?.addEventListener(`click`,async()=>{let e=H?.value||``,t=U?.value||``,n=W?.value||``;J(``),Y(``);let r=d(t);if(r){J(r,!0);return}if(t!==n){J(`New passwords do not match.`,!0);return}G.disabled=!0;try{await l(e,t),Z(!1),Y(`Password updated.`)}catch(e){J(e.message||`Could not update password`,!0)}finally{G.disabled=!1}})}export{_ as PRIVACY_POLICY_URL,S as bindSettingsPanel,C as renderSettingsTab,y as settingsSectionHtml};