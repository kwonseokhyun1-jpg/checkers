import{$ as e,A as t,C as n,D as r,E as i,G as a,J as o,L as s,M as c,N as l,O as u,P as d,Q as f,S as p,U as m,W as h,X as g,Y as _,_ as v,b as y,ct as b,dt as x,et as S,ht as C,k as w,m as T,q as E,rt as D,st as O,v as ee,w as k,x as te,y as A}from"./storage-rDhtNKuP.js";import{a as ne,c as j,o as M}from"./auth-DxjpTlkZ.js";import{U as N,c as P,i as F,l as I,n as re,o as ie,r as L,s as R,t as z,u as B}from"./index-BHHHxzNY.js";import{playCosmeticOpenAnimation as V}from"./cosmeticOpenAnimation-Ddqa032w.js";import{notifyUnlockTutorial as H}from"./tutorialUnlocks-kHlUdeEI.js";var U=null;function W(){U!=null&&(clearInterval(U),U=null)}function G(e){if(!e)return;W();let t=()=>{e.textContent=`Resets in: ${te(k())}`};t(),U=setInterval(t,1e3)}function K(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var q={common:`rarity-common`,uncommon:`rarity-uncommon`,rare:`rarity-rare`,epic:`rarity-epic`,legendary:`rarity-legendary`},J={common:0,uncommon:1,rare:2,epic:3,legendary:4},ae={others:`Exclude others`,unowned:`Exclude unowned`,owned:`Exclude owned`,starters:`Exclude starters`};function oe({type:e,excludeOptions:t,profile:n}){let r=n.cosmetics?.owned||{};return f.filter(n=>{if(t.others&&n.type!==e)return!1;let i=r[n.type]?.includes(n.id)??!1;return!(t.unowned&&!i||t.owned&&i||t.starters&&D.has(n.id))}).sort((e,t)=>{let n=+!r[e.type]?.includes(e.id),i=+!r[t.type]?.includes(t.id);return n===i?(J[e.rarity]??9)-(J[t.rarity]??9)||e.name.localeCompare(t.name):n-i})}function se(e){if(e===`pieceSkin`)return`Piece skins`;if(e===`frame`)return`Frames`;let t=R(e);return t.endsWith(`s`)?t:`${t}s`}function Y(e){let t=E(e);return t?_(t):``}var ce={pvp:`<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <div class="profile-hero-card">
      <div class="profile-showcase profile-hero-card__showcase">
        <div class="profile-showcase__banner" id="profile-banner-preview" style="background:${F(e.equipped.banner)}"></div>
        <div class="profile-showcase__hero profile-hero-card__hero">
          <div class="profile-avatar-stack ${P(e.equipped.frame)}" id="profile-avatar-stack">
            <div class="profile-avatar-inner" id="profile-avatar-preview" aria-hidden="true">${I(e.equipped.avatar)||`<span class="profile-avatar-fallback">${K(a)}</span>`}</div>
          </div>
      ...
      <span class="profile-achievement-card__status">${v}</span>`,f){let i=document.createElement(`button`);i.type=`button`,i.className=`btn-primary profile-achievement-card__claim`,i.textContent=`Claim Title`,i.addEventListener(`click`,()=>{t(e,r.id).success&&(T(e),Z(e,n,{onTitleChanged:a}),a?.())}),m.appendChild(i)}n.appendChild(m)}}function Q(e,t,{onCurrencyChange:r}={}){if(!t)return;t.innerHTML=``;let i=[...p(e)].sort((t,n)=>{let r=e=>e.canClaim?0:e.claimed?2:1,i=r(t)-r(n);return i===0?A(e,n.templateId)-A(e,t.templateId):i});for(let{template:a,templateId:o,complete:s,claimed:c,canClaim:l}of i){if(!a)continue;let i=!s&&!c,u=document.createElement(`article`);u.className=[`profile-achievement-card`,`daily-quest-card`,i?`profile-achievement-card--locked`:``,s?`profile-achievement-card--complete`:``,c?`profile-achievement-card--claimed`:``,l?`profile-achievement-card--claimable`:``].filter(Boolean).join(` `);let d=ee(e,o),f=a.target?Math.min(100,Math.round(n(e,o)/a.target*100)):0,p=y(o),m=a.reward.currency===`stars`?`daily-quest-card__reward--stars`:`daily-quest-card__reward--gems`,h=c?`Claimed`:l?`Claim reward`:s?`Complete`:`In progress`;if(u.innerHTML=`
      <div class="profile-achievement-card__head">
      ...
      <span class="profile-achievement-card__status">${h}</span>`,l){let n=document.createElement(`button`);n.type=`button`,n.className=`btn-primary profile-achievement-card__claim`,n.textContent=a.reward.currency===`stars`?`Claim stars`:`Claim gems`,n.addEventListener(`click`,()=>{v(e,o).success&&(T(e),Q(e,t,{onCurrencyChange:r}),r?.())}),u.appendChild(n)}t.appendChild(u)}}function $(e,t,{onTitleChanged:n,onCurrencyChange:r}={}){if(!t)return;W(),t.innerHTML=`
            ...
            <div id="profile-exclude-menu-panel" class="profile-exclude-menu__panel hidden" role="menu" hidden>
              ${Object.entries(ae).map(([e,t])=>`
                <label class="profile-exclude-menu__option" role="menuitemcheckbox">
                  <input type="checkbox" data-exclude-option="${e}" ${S[e]?`checked`:``} />
                  <span>${t}</span>
                </label>`).join(``)}
            </div>
          </div>
  ...
  `;let u=n.querySelector(`#profile-cosmetic-grid`),d=`avatar`,f=``,p={...S},g=n.querySelector(`#profile-exclude-menu-btn`),_=n.querySelector(`#profile-exclude-menu-panel`),v=e=>{!g||!_||(g.setAttribute(`aria-expanded`,e?`true`:`false`),_.classList.toggle(`hidden`,!e),_.hidden=!e)};g?.addEventListener(`click`,e=>{e.stopPropagation(),v(_?.hidden??!0)}),_?.querySelectorAll(`[data-exclude-option]`).forEach(e=>{e.addEventListener(`change`,()=>{let t=e.dataset.excludeOption;!t||!(t in p)||(p[t]=e.checked,x())})}),n.addEventListener(`click`,()=>v(!1)),_?.addEventListener(`click`,e=>e.stopPropagation());let y=()=>{let e=b(t);n.querySelector(`#profile-banner-preview`).style.background=F(e.equipped.banner);let r=n.querySelector(`#profile-avatar-preview`),a=(f||`P`).charAt(0).toUpperCase();r&&(r.innerHTML=I(e.equipped.avatar)||`<span class="profile-avatar-fallback">${K(a)}</span>`);let o=n.querySelector(`#profile-avatar-stack`);o&&(o.className=`profile-avatar-stack ${P(e.equipped.frame)}`);let s=n.querySelector(`#profile-hero-username`);s&&(s.textContent=f||`Player`);let c=n.querySelector(`.profile-hero-card__title-badge`),l=Y(t);if(l)if(c)c.innerHTML=l;else{let e=n.querySelector(`.profile-hero-card__name-row`);if(e){let t=document.createElement(`span`);t.className=`profile-hero-card__title-badge`,t.innerHTML=l,e.appendChild(t)}}else c&&c.remove();i?.()},x=()=>{let e=oe({type:d,excludeOptions:p,profile:t});u.innerHTML=``;for(let n of e){let{id:e,type:r}=n,i=t.cosmetics?.owned?.[r]?.includes(e)??!1,a=t.cosmetics.equipped[r]===e,o=document.createElement(`button`);o.type=`button`,o.disabled=!i,o.className=[`profile-cosmetic-card`,q[n.rarity]||``,a?`profile-cosmetic-card--equipped`:``,i?``:`profile-cosmetic-card--locked`].filter(Boolean).join(` `);let s=B(e,n.type),c=a?`Equipped`:i?`Equip`:`Locked`;o.innerHTML=`
        ...
        <span class="profile-cosmetic-card__action">${c}</span>`,o.addEventListener(`click`,()=>{!i||a||O(t,r,e).success&&(T(t),x(),y(),H(`cosmetic-equipped`,{type:r,id:e}))}),u.appendChild(o)}e.length||(u.innerHTML=`<p class="muted">No cosmetics match these filters. Try changing Exclude options.</p>`)};for(let e of n.querySelectorAll(`.profile-filter-btn`))e.addEventListener(`click`,()=>{d=e.dataset.cosFilter,n.querySelectorAll(`.profile-filter-btn`).forEach(t=>t.classList.toggle(`active`,t===e)),x()});n.querySelector(`.profile-filter-btn`)?.classList.add(`active`),x();let C=()=>{let e=n.querySelector(`#profile-title-grid`);if(!e)return;e.innerHTML=``;let r=E(t);for(let n of m){let s=o(t,n.id),c=r===n.id,l=document.createElement(`button`);l.type=`button`,l.disabled=!s,l.className=[`profile-title-card`,h[n.rarity]||``,`profile-title-card--glow-${n.glow}`,s?``:`profile-title-card--locked`,c?`profile-title-card--equipped`:``].filter(Boolean).join(` `),l.innerHTML=`
        ...
        <span class="profile-title-card__action">${c?`Equipped`:s?`Equip`:`Locked`}</span>`,l.addEventListener(`click`,()=>{s&&a(t,c?null:n.id).success&&(T(t),C(),y(),i?.())}),e.appendChild(l)}},w=e=>{n.querySelectorAll(`.profile-section-tab`).forEach(t=>{t.classList.toggle(`active`,t.dataset.profileSection===e)});let t=n.querySelector(`#profile-section-cosmetics`);t&&(t.classList.toggle(`hidden`,e!==`cosmetics`),t.hidden=e!==`cosmetics`);let r=n.querySelector(`#profile-section-titles`);r&&(r.classList.toggle(`hidden`,e!==`titles`),r.hidden=e!==`titles`),e===`titles`&&C()};for(let e of n.querySelectorAll(`.profile-section-tab`))e.addEventListener(`click`,()=>w(e.dataset.profileSection));C(),l&&(async()=>{try{f=(await ne(c.id))?.username||c.user_metadata?.display_name||``,y()}catch(e){console.warn(`Could not load profile username`,e)}})()}export{z as headerProfileAvatarHtml,de as renderAchievementsTab,X as renderCosmeticBoxes,fe as renderProfileTab,$ as renderQuestsTab,re as resolveDisplayUsername};