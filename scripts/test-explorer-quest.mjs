/**
 * Unit tests for Explorer quest (clear Adventure chapter 5).
 */
import {
  syncExplorer,
...
function testIncomplete() {
  ...
  assert(profile.achievements.progress.explorer === 3, "progress should mirror chapter 5 clears");
  assert(!isAchievementComplete(profile, "explorer"), "3 clears should not complete explorer");
  assert(progressLabel(profile, "explorer") === "3 / 10 chapter 5 stages", "progress label should show clears");
...
function testIgnoresOtherChapters() {
  ...
  syncExplorer(profile);
  assert(countChapter5StagesCleared(profile) === 0, "chapter 1 clears should not count");
