...
| Item | Purpose |
|------|---------|
| `css/mobile-game.css` | Bottom tab bar, safe areas, touch targets, match UI polish, install banner, chest reveals, deck editor, profile/quests, mystery box, PvP history, account settings, prebattle, spell cast HUD, card preview actions, adventure scroll, shop pulls, shell HUD, shop chest layout, board row/column picks, PvP join/waiting, splash safe areas |
